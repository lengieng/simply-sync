...
class ContactManager extends APIResource {
  ...
  /**
   * Create a new contact with mergeColumn option (upsert).
   *
   * @param array $fields  An array of arrays with keys 'id' and 'value',
   *   where 'id' is a column ID and value is the value to set.
   * @param integer $mergeColumn  Column to merge.
   * @param array $listIds If provided, allows a way to easily assign the
   *   new contact to one or more lists, by list ID.
   *
   * @return array A representation of the newly created contact.
   */
  public function createContact2($fields, $mergeColumn = null, $listIds = array())
  {
      if (is_scalar($listIds)) {
          $listIds = array($listIds);
      }
      ...
      return $this->request(
          ...
          isset($mergeColumn) ? array('mergecolumn' => intval($mergeColumn)) : false,
          $data);
  }

  /**
   * Create contact with batch.
   *
   * @param array $contacts  An array of contacts, where contact is an array of
   *   arrays with keys 'id' and 'value', where 'id' is a column ID and value
   *   is the value to set.
   * @param array $listIds If provided, allows a way to easily assign the
   *   new contact to one or more lists, by list ID.
   *
   ...
   */
  public function createContactBatch($contacts, $mergeColumn = null, $listIds = array())
  {
      if (is_scalar($listIds)) {
          $listIds = array($listIds);
      }
      ...
      foreach($contacts as $contact) {
          if (count($listIds) > 0) {
              $data[] = array('contact' => array('fields' => $contact, 'lists' => $listIds));
          } else {
              $data[] = array('contact' => array('fields' => $contact));
          }
      }

      return $this->request(
          'POST',
          'contactmanager/contacts/batch',
          isset($mergeColumn) ? array('mergeColumn' => intval($mergeColumn)) : false,
          $data
      );
  }
  ...
  /**
   * Retrieve contact batch information.
   *
   * @param string $id  batch id
   *
   * @return array Information about the batch that was submitted.
   */
  public function getContactBatchInfo($id) {
